...
def render_styled_table(
    ...
    custom_formats: Optional[Dict[str, str]] = None,
) -> None:
    ...
    for col in df.columns:
        if custom_formats and col in custom_formats:
            ...
            col_l = col.lower()
            if any(m in col_l for m in MONTH_NAMES):
                active_fmt[col] = "{:+.1%}"
            elif "return" in col_l or "rate" in col_l or "yield" in col_l or "alpha" in col_l:
                active_fmt[col] = "{:+.1%}"
        ...
        for col in df.columns:
            ...
            if "SYMBOL" in c_str:
                ...
                if any(w in c_str for w in ["DELTA", "Δ"]):
                    clr = "badge-green" if val > 0 else ("badge-red" if val < 0 else "badge-neutral")
                    symbol = "▲" if val > 0 else ("▼" if val < 0 else "—")
                    cells_html.append(f'<td class="td-center"><span class="badge-pill {clr}">{symbol} {abs(int(round(val)))}</span></td>')
                elif any(w in c_str for w in ["RANK", "STOCKS", "COUNT", "HOLDINGS", "TRADES"]):
                    cells_html.append(f'<td class="td-center"><strong>{int(round(val))}</strong></td>')
