...
def render_config_view(rank_df: pd.DataFrame) -> None:
    ...
    with sync_c2:
        ...
        with btn_c1:
            if st.button("Sync NSE CSVs", type="primary", use_container_width=True, key="btn_sync_indices"):
                with st.spinner("Downloading official index CSVs from niftyindices.com…"):
                    ...
                    st.rerun()
        with btn_c2:
            if st.button("Purge Cache", type="secondary", use_container_width=True, key="btn_force_sync_top"):
                st.session_state["force_refresh"] = True
                st.session_state.pop("data_loaded_key", None)
                st.cache_data.clear()
                st.rerun()
