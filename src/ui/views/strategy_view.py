...
def compute_multi_strategy_monthly_matrix(
    ...
    """
    Computes rigorous walk-forward monthly returns & cumulative trajectories for all quantitative models:
    ...
    valid_month_ends = [d for d in month_ends if d in prices.index or prices.index.get_indexer([d], method="pad")[0] > 0]

    if len(valid_month_ends) < n_months + 1:
        rebal_indices = list(range(max(0, len(prices) - (n_months * 21) - 1), len(prices) - 1, 21))
    ...
    month_cols = []
    strat_monthly_rets = {k: [] for k in strategies}
    bench_monthly_rets = []

    # Daily trajectory tracking
    all_daily_dates = []
    strat_daily_rets = {k: [] for k in strategies}
    bench_daily_rets = []
    ...
    for p_idx in range(len(rebal_dates) - 1):
        ...
        # 3. Industry-Relative Momentum
        ind_map = _rank_df.set_index("Symbol")["Industry"].to_dict() if "Industry" in _rank_df.columns else {}
        ind_scores = {}
        ...
        ind_rel_score = comp_score - comp_score.index.map(lambda s: ind_means.get(ind_map.get(s, "General"), 0))

        # 4. Momentum Acceleration (Short vs Long)
        ret_1m = (p_slice.iloc[-1] / p_slice.iloc[-min(21, len(p_slice))]) - 1
        ret_3m = (p_slice.iloc[-1] / p_slice.iloc[-min(63, len(p_slice))]) - 1
        ret_12m = (p_slice.iloc[-1] / p_slice.iloc[-min(252, len(p_slice))]) - 1
    ...
    rows = []
    bench_cum = np.prod([1 + r for r in bench_monthly_rets]) - 1 if bench_monthly_rets else 0.0
    for name in strategies:
        m_rets = strat_monthly_rets[name]
        cum_ret = np.prod([1 + r for r in m_rets]) - 1 if m_rets else 0.0
    ...
    # Add Benchmark Row
    b_dict = {"Strategy Model": "🏛️ Benchmark (Nifty 500)"}
    ...
    if all_daily_dates:
        ...
        curves["🏛️ Benchmark (Nifty 500)"] = (1 + pd.Series(bench_daily_rets[:len(d_idx)], index=d_idx)).cumprod()
        for name in strategies:
...
def render_strategy_view(
    ...
    top_n = c_slider.slider("Top-N Consensus Threshold", 20, 100, 50, 5, key="overlay_top_n_slider")
    ...
    if not overlap.empty:
        disp_cols = [
            "Composite Rank", "Symbol", "Industry", "CMP",
            "Sharpe Rank", "Residual Rank", "Ind-Rel Rank", "Accel Rank",
            "3M Return", "6M Return", "Persistence", "ATR %",
        ]
        ...
        st.info(f"No stock qualifies in the top {top_n} of all three systems. Try raising the threshold slider.")
    ...
    systems = [
        (e1, "Residual (α) Momentum", "Stock returns regressed against market index. Extracts idiosyncratic alpha after stripping market beta. Low correlation to broad market swings.", "#4f46e5"),
        (e2, "Industry-Relative Momentum", "Stock composite Sharpe×R² minus industry peer average. Isolates top sector outperformers independently of sector cycle.", "#0284c7"),
        (e3, "Momentum Acceleration", "Short-term momentum (1M+3M+6M) minus long-term momentum (9M+12M). Detects accelerating velocity and early-stage breakouts.", "#059669"),
    ]
    ...
    st.markdown("##### 📊 Multi-Strategy Monthly Backtest & Alpha Attribution (Last 6 Months)")
    st.caption("Walk-forward historical performance comparison across quantitative momentum models with zero look-ahead bias.")

    with st.spinner("Computing 6-month walk-forward multi-strategy matrix…"):
        matrix_df, curves = compute_multi_strategy_monthly_matrix(ph, adj_close, rank_df, top_n=20, n_months=6)

    if not matrix_df.empty:
        # Find the #1 performing model
        non_bench = matrix_df[matrix_df["Strategy Model"] != "🏛️ Benchmark (Nifty 500)"]
        if not non_bench.empty:
            best_model_row = non_bench.sort_values("6M Net Return", ascending=False).iloc[0]
            best_name = best_model_row["Strategy Model"]
            best_ret = best_model_row["6M Net Return"] * 100
            best_alpha = best_model_row["6M Alpha"] * 100
    ...
    with st.expander("📋 Full Multi-Strategy Overlay Ranking Table"):
        full_cols = [
            "Composite Rank", "Symbol", "Industry",
            "Sharpe Rank", "Residual Rank", "Ind-Rel Rank", "Accel Rank",
            "In All Top", "CMP", "3M Return",
        ]
        render_saas_table(
