...
from src.engine.breadth import compute_hl_timeseries, compute_ma_breadth, get_recent_hl_events
...
def render_breadth_view(rank_df: pd.DataFrame, adj_close: pd.DataFrame) -> None:
    ...
    ma_type = bc1.segmented_control("MA Type", ["SMA", "EMA"], default="EMA", key="br_ma_type", label_visibility="collapsed")
    if not ma_type:
        ma_type = "EMA"
    sel_mas = bc2.multiselect("MA Periods", ["10D", "20D", "50D", "100D", "200D"], default=["50D", "200D"], key="br_sel_mas", placeholder="Select MA Periods…", label_visibility="collapsed")
    history_days = bc3.selectbox("Lookback", [63, 126, 252], index=1, format_func=lambda x: {63: "3 Months", 126: "6 Months", 252: "1 Year"}[x], key="br_lb_days", label_visibility="collapsed")
    bview = bc4.segmented_control("Breakdown", ["Universe", "By Index"], default="Universe", key="br_bview", label_visibility="collapsed")
    if not bview:
        bview = "Universe"
    ...
    breadth_df = compute_ma_breadth(ph, adj_close, tuple(sel_mas), lookback=history_days, ma_type=ma_type)
    ...
    if not breadth_df.empty:
        ...
        for ma_lbl in sel_mas:
            if ma_lbl in breadth_df.columns:
                ...
                clr = "#059669" if val >= 60 else ("#e11d48" if val <= 40 else "#d97706")
                sig = "Strong Bullish" if val >= 60 else ("Weak / Deteriorating" if val <= 40 else "Neutral Participation")
        ...
        st.html(f'<div style="display: grid; grid-template-columns: repeat({len(kpi_items)}, 1fr); gap: 10px; margin-bottom: 12px;">{"".join(kpi_items)}</div>')
        ...
        if bview == "Universe":
            ...
            idx_order = ["NIFTY 50", "NIFTY NEXT 50", "NIFTY MIDCAP 150", "NIFTY SMALLCAP 250", "NIFTY MICROCAP 250"]
            for idx_name in idx_order:
                syms = rank_df[rank_df["Indices"].str.contains(idx_name.replace("NIFTY ", ""), na=False)]["Symbol"].tolist()
                valid_syms = [s for s in syms if s in adj_close.columns]
                if not valid_syms:
                    continue
                ma_s = adj_close[valid_syms].ewm(span=50).mean() if ma_type == "EMA" else adj_close[valid_syms].rolling(50).mean()
                pct = float(((adj_close[valid_syms].iloc[-1] > ma_s.iloc[-1]).sum() / len(valid_syms)) * 100)
                clr = "#059669" if pct >= 60 else ("#e11d48" if pct <= 40 else "#d97706")
    ...
    st.caption("Measures daily expansion of new highs vs new lows. Divergences frequently precede broad index turning points.")

    h1, h2, h3 = st.columns(3)
    hl_window = h1.selectbox("High/Low Window", [52, 126, 252], index=2, format_func=lambda x: {52: "52 Days (Quarter)", 126: "126 Days (6 Months)", 252: "252 Days (52 Weeks)"}[x], key="hl_win_sel")
    hl_history = h2.selectbox("History Period", [63, 126, 252], index=1, format_func=lambda x: {63: "3 Months", 126: "6 Months", 252: "1 Year"}[x], key="hl_hist_sel")
    hl_disp = h3.segmented_control("Display Format", ["% of Universe", "Stock Count"], default="% of Universe", key="hl_fmt_radio")
    if not hl_disp:
        hl_disp = "% of Universe"
    ...
    if not hl_df.empty:
        ...
        ratio_clr = "#059669" if hl_ratio > 2 else ("#dc2626" if hl_ratio < 0.5 else "#d97706")
        ratio_sig = "Bullish Expansion" if hl_ratio > 2 else ("Bearish Contraction" if hl_ratio < 0.5 else "Neutral")
        ...
        with st.expander("📊 Net New Highs (Highs − Lows) & Breakout Stocks by Date", expanded=True):
            ...
            fig_net.update_layout(
                ...
                plot_bgcolor="#ffffff",
                font=dict(family="Plus Jakarta Sans, sans-serif", size=10, color="#334155"),
                title=dict(text="<b>Daily Net New Highs (Highs − Lows)</b>", font=dict(size=13, color="#0f172a")),
            ...
            if not hl_events_df.empty:
                ef1, ef2 = st.columns([1.5, 2.5], vertical_alignment="center")
                ev_sel = ef1.segmented_control("Filter Events", ["All Events", "🟢 52W Highs", "🔴 52W Lows"], default="All Events", key="hl_ev_filter", label_visibility="collapsed")
                if ev_sel == "🟢 52W Highs":
                    disp_events = hl_events_df[hl_events_df["Event"].str.contains("High")]
                elif ev_sel == "🔴 52W Lows":
                    disp_events = hl_events_df[hl_events_df["Event"].str.contains("Low")]
                else:
                    disp_events = hl_events_df

                render_saas_table(disp_events, key="hl_breakout_records_table", max_height=400)
            else:
