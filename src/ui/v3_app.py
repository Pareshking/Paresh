...
def _rankings(_calc: MomentumEngine, idx: pd.DataFrame, mcaps: Any, close: pd.DataFrame, high: pd.DataFrame, weights: tuple[float, ...]):
    # The engine is a cached resource; only the serializable ranking result is cached here.
...
def load_data():
    ...
    weights = tuple(float(st.session_state.get(f"v3_w{i}", w)) for i, w in enumerate(WEIGHTS, 1))
    total = sum(weights)
    weights = tuple(w / total for w in weights) if total else WEIGHTS
...
def _context_strip(df: pd.DataFrame, regime: Any) -> None:
    total = len(df)
    ema = int(df.get("Above 50 EMA", pd.Series(dtype=object)).map(_bool).sum())
    near = int(df.get("Near 52W High", pd.Series(dtype=object)).map(_bool).sum())
    top = int((pd.to_numeric(df.get("Rank", pd.Series(dtype=float)), errors="coerce") <= 50).sum())
    hv = int(df.get("Volume", pd.Series(dtype=object)).astype(str).str.lower().isin(["high", "surge"]).sum())
    breadth = ema / total * 100 if total else 0
    status = getattr(regime, "status", None) or "—"
...
def _filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    with search_col:
        query = st.text_input("Search", placeholder="Search symbol or industry", type="search", label_visibility="collapsed", key="v3_search")
    with preset_col:
    ...
    out = out.sort_values(sort, ascending=(sort == "Rank"), na_position="last")
    return out


def _mobile_card(row: pd.Series) -> str:
    rank = _num(row.get("Rank")); rank_text = f"#{int(rank)}" if rank is not None else "—"
    symbol = html.escape(str(row.get("Symbol", "—"))); industry = html.escape(str(row.get("Industry", "—")))
    cmp_text = _money(row.get("CMP")); day = _num(row.get("1D Change")); day_text = "—" if day is None else f"{day:+.1f}% 1D"; day_cls = "v3-pos" if day is not None and day >= 0 else "v3-neg"
    pct = max(0, min(100, _num(row.get("Score Percentile")) or 0)); score = _num(row.get("Score")); score_text = "—" if score is None else f"{score:.3f}"
    metrics = [("1M", _ret(row.get("1M Return"))), ("3M", _ret(row.get("3M Return"))), ("6M", _ret(row.get("6M Return"))), ("12M", _ret(row.get("12M Return"))), ("3M S", f'{_num(row.get("3M Sharpe")):.2f}' if _num(row.get("3M Sharpe")) is not None else "—"), ("52W", _pct(row.get("% High"))), ("EMA", _pct(row.get("% 50 EMA"), True)), ("12M DD", _pct(row.get("Max DD 12M"), False))]
    mh = "".join(f'<div><div class="v3-mini-label">{a}</div><div class="v3-mini-value">{b}</div></div>' for a, b in metrics)
...
def render_screener(data: dict[str, Any]) -> None:
    ...
    st.markdown('<div class="v3-brand"><div class="v3-mark">PQ</div><div><div class="v3-brand-name">PARESH QUANT</div><div class="v3-brand-sub">System-1 · Sharpe Momentum · live ranking</div></div></div>', unsafe_allow_html=True)
    st.markdown('<div class="v3-title">Screener</div><div class="v3-sub">Find the strongest quantitative setups first. Detail belongs on the stock page; this screen is optimized for comparison.</div>', unsafe_allow_html=True)
    _context_strip(df, data.get("regime"))
    filtered = _filters(df)
    st.markdown(f'<div class="v3-section">{len(filtered):,} matching stocks <span class="v3-count">· ranked by {html.escape(st.session_state.get("v3_sort", "Rank"))}</span></div>', unsafe_allow_html=True)
    if filtered.empty:
        st.info("No stocks match the current filters.")
        return
    shown = filtered.head(100)

    # Desktop/tablet: compact comparison table. Mobile: deliberately different card hierarchy.
    with st.container(key="v3-desktop-table"):
        display = ["Rank", "Symbol", "Industry", "CMP", "1D Change", "1M Return", "3M Return", "6M Return", "12M Return", "3M Sharpe", "% High", "% 50 EMA", "Volume"]
        display = [c for c in display if c in shown.columns]
        cfg = {
        ...
        event = st.dataframe(shown[display].reset_index(drop=True), column_config=cfg, hide_index=True, use_container_width=True, height=min(620, 48 + len(shown) * 35), on_select="rerun", selection_mode="single-row", key="v3_table")
    ...
    with st.container(key="v3-mobile-list"):
        ...
        for i, (_, row) in enumerate(shown.iterrows()):
            ...
            if st.button("View", key=f"v3_mobile_{symbol}_{i}", use_container_width=True):
...
def render_stock(data: dict[str, Any], symbol: str) -> None:
    ...
    row = found.iloc[0]
    sym = str(row["Symbol"])
    if st.button("← Back to Screener", key="v3_back", type="tertiary"):
        st.session_state.v3_page = "Screener"
        st.rerun()
    st.markdown(f'<div class="v3-detail-hero"><div class="v3-detail-row"><div><div class="v3-detail-rank">RANK #{_num(row.get("Rank")):.0f}</div><div class="v3-detail-symbol">{html.escape(sym)}</div><div class="v3-detail-meta">{html.escape(str(row.get("Industry", "—")))} · {html.escape(str(row.get("Indices", "—")))}</div></div><div><div class="v3-detail-price">{_money(row.get("CMP"))}</div><div class="v3-day {"v3-pos" if (_num(row.get("1D Change")) or 0)>=0 else "v3-neg"}">{_ret(_num(row.get("1D Change"))/100 if _num(row.get("1D Change")) is not None else None)} 1D</div></div></div><div class="v3-badges">{_badges(row)}</div></div>', unsafe_allow_html=True)

    pctl = _num(row.get("Score Percentile")) or 0
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(f'<div class="v3-kpi"><div class="v3-kpi-label">Score</div><div class="v3-kpi-value">{_num(row.get("Score")):.3f}</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="v3-kpi"><div class="v3-kpi-label">Percentile</div><div class="v3-kpi-value">{pctl:.0f}</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="v3-kpi"><div class="v3-kpi-label">Rank Δ 1M</div><div class="v3-kpi-value">{_num(row.get("Rank Δ 1M")):+.0f}</div></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="v3-kpi"><div class="v3-kpi-label">Rank Δ 3M</div><div class="v3-kpi-value">{_num(row.get("Rank Δ 3M")):+.0f}</div></div>', unsafe_allow_html=True)

    st.markdown('<div class="v3-section">Price structure</div>', unsafe_allow_html=True)
    try:
        import plotly.graph_objects as go
        close = pd.to_numeric(data["close"][sym], errors="coerce").dropna()
        fig = go.Figure()
        if data.get("open") is not None and data.get("high") is not None and data.get("low") is not None:
            o = pd.to_numeric(data["open"][sym], errors="coerce").reindex(close.index)
            h = pd.to_numeric(data["high"][sym], errors="coerce").reindex(close.index)
            l = pd.to_numeric(data["low"][sym], errors="coerce").reindex(close.index)
            fig.add_trace(go.Candlestick(x=close.index, open=o, high=h, low=l, close=close, name="Price"))
        else:
            fig.add_trace(go.Scatter(x=close.index, y=close, mode="lines", name="Price"))
        fig.add_trace(go.Scatter(x=close.index, y=close.ewm(span=50, adjust=False).mean(), mode="lines", name="50 EMA"))
        for col, label in (("52W High", "52W High"), ("ATH", "ATH")):
            value = _num(row.get(col))
            if value is not None:
                fig.add_hline(y=value, annotation_text=label)
        fig.update_layout(height=430, margin=dict(l=4, r=4, t=10, b=4), xaxis_rangeslider_visible=False, template="plotly_white", legend=dict(orientation="h"))
        st.plotly_chart(fig, use_container_width=True)
    except Exception as exc:
        st.warning(f"Chart unavailable: {exc}")

    st.markdown('<div class="v3-section">Momentum matrix</div>', unsafe_allow_html=True)
    matrix = pd.DataFrame([{ "Window": f"{p}M", "Return": _ret(row.get(f"{p}M Return")), "Sharpe": _num(row.get(f"{p}M Sharpe")), "Max Drawdown": _pct(row.get(f"Max DD {p}M"), False)} for p in PERIODS])
    st.dataframe(matrix, use_container_width=True, hide_index=True, column_config={"Sharpe": st.column_config.NumberColumn(format="%.2f")})

    st.markdown('<div class="v3-section">Trend · highs · risk</div>', unsafe_allow_html=True)
    trend = pd.DataFrame([
        ("50 EMA distance", _pct(row.get("% 50 EMA"))), ("52W High", _money(row.get("52W High"))), ("52W High Date", row.get("52W High Date", "—")),
        ("From 52W High", _pct(row.get("% High"))), ("ATH", _money(row.get("ATH"))), ("ATH Date", row.get("ATH Date", "—")),
        ("From ATH", _pct(row.get("% ATH"))), ("6M Persistence", _pct(row.get("6M Persistence"), False)), ("ATR", _money(row.get("ATR"))),
        ("ATR%", _pct(row.get("ATR%"), False)), ("Stop Loss", _money(row.get("Stop Loss"))), ("Chandelier Exit", _money(row.get("Chand Exit"))),
        ("12M Max Drawdown", _pct(row.get("Max DD 12M"), False)), ("Volume", row.get("Volume", "—")),
    ], columns=["Metric", "Value"])
    st.dataframe(trend, use_container_width=True, hide_index=True)

    st.markdown('<div class="v3-section">Metadata · data quality</div>', unsafe_allow_html=True)
    meta = pd.DataFrame([(k, row.get(k, "—")) for k in ("Market Cap (Cr)", "Industry", "Indices", "Short History", "FFill %", "Data Gap")], columns=["Field", "Value"])
...
def run() -> None:
    ...
    data = load_data()
    if data is None:
        st.error("Unable to load the quantitative dataset.")
        return
    page = st.segmented_control("Page", ["Screener", "Stock Detail"], default=st.session_state.v3_page, key="v3_page_control", width="content")
    st.session_state.v3_page = page
    if page == "Stock Detail" and st.session_state.get("v3_symbol"):
        render_stock(data, str(st.session_state.v3_symbol))
    else:
        st.session_state.v3_page = "Screener"
