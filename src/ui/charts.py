...
def render_candlestick_drilldown(
    ...
    with c_chart:
        ...
        if len(_close) >= 20:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=ema50.values,
                    mode="lines",
                    line=dict(color="#4f46e5", width=1.2, dash="dot"),
                    name="50 EMA",
                ),
                row=1,
                col=1,
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Price (₹)", gridcolor="#f1f5f9", zeroline=False),
            yaxis2=dict(title="Volume", gridcolor="#f1f5f9", zeroline=False),
            yaxis3=dict(title="RSI (14)", range=[0, 100], gridcolor="#f1f5f9", zeroline=False),
...
def render_sector_treemap(
    ...
    fig.update_traces(
        ...
        textfont=dict(family="IBM Plex Mono, monospace", size=11, color="#0f172a"),
        marker=dict(
            cornerradius=4,
    ...
    fig.update_layout(
        ...
        coloraxis_colorbar=dict(
            title=dict(text=f"{return_col}", font=dict(family="IBM Plex Mono", size=10, color="#475569")),
            tickformat="+.0%",
            thickness=12,
            len=0.7,
...
def render_rrg_chart(
    rrg_df: pd.DataFrame,
    highlight_industries: Optional[List[str]] = None,
    ...
    fig.add_annotation(
        ...
        xanchor="left",
        yanchor="top",
        font=dict(size=14, color="#3b82f6", family="Plus Jakarta Sans, sans-serif"),
        opacity=0.9,
    )
    # Leading (Top-Right)
    fig.add_annotation(
        ...
        xanchor="right",
        yanchor="top",
        font=dict(size=14, color="#15803d", family="Plus Jakarta Sans, sans-serif"),
        opacity=0.9,
    )
    # Lagging (Bottom-Left)
    fig.add_annotation(
        ...
        xanchor="left",
        yanchor="bottom",
        font=dict(size=14, color="#dc2626", family="Plus Jakarta Sans, sans-serif"),
        opacity=0.9,
    )
    ...
    fig.add_annotation(
        ...
        xanchor="right",
        yanchor="bottom",
        font=dict(size=14, color="#ca8a04", family="Plus Jakarta Sans, sans-serif"),
        opacity=0.9,
    )
    ...
    fig.add_annotation(
        ...
        xanchor="center",
        yanchor="middle",
        font=dict(size=12, color="#94a3b8", family="Plus Jakarta Sans, sans-serif"),
        opacity=0.35,
    )
        ...
        for _, row in rrg_df.iterrows():
            ...
            if t_r and t_m and len(t_r) > 1:
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=t_m,
                        mode="lines",
                        line=dict(color="#cbd5e1", width=1, shape="spline", smoothing=1.3),
                        opacity=0.25,
                        showlegend=False,
                        hoverinfo="skip",
                    )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[row["RS_Momentum"]],
                    mode="markers",
                    marker=dict(size=4, color="#cbd5e1", opacity=0.3),
                    showlegend=False,
                    hoverinfo="skip",
                )
            )
    ...
    for idx, (_, row) in enumerate(rrg_df.iterrows()):
        ...
        if trail_r and trail_m and len(trail_r) > 1 and len(trail_m) > 1:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=trail_m,
                    mode="lines+markers",
                    line=dict(color=sec_clr, width=2.5, shape="spline", smoothing=1.3),
                    marker=dict(size=6, color=sec_clr, symbol="circle"),
                    opacity=0.90,
                    showlegend=False,
                    hovertemplate=(
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[row["RS_Momentum"]],
                mode="markers+text",
                marker=dict(size=12, color=sec_clr, line=dict(width=2, color="#ffffff")),
                text=[f" <b>{row['Industry']}</b>"],
                textposition="top center",
                textfont=dict(size=10.5, color=sec_clr, family="Plus Jakarta Sans, sans-serif"),
                name=f"{row['Industry']}",
                showlegend=True,
                hovertemplate=(
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            dtick=1.0,
            tickangle=-45,
            tickfont=dict(family="IBM Plex Mono", size=9.5, color="#64748b"),
        ),
        yaxis=dict(
        ...
        legend=dict(
            ...
            xanchor="center",
            x=0.5,
            font=dict(family="IBM Plex Mono", size=10, color="#334155"),
        ),
        height=640,
        margin=dict(l=45, r=45, t=25, b=65),
        hovermode="closest",
    )
...
def render_breadth_chart(breadth_df: pd.DataFrame, ma_type: str = "SMA") -> None:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#ffffff",
        font=dict(family="Plus Jakarta Sans, sans-serif", size=11, color="#334155"),
        title=dict(text=f"<b>Market Breadth (% Stocks Above {ma_type})</b>", font=dict(size=14, color="#0f172a")),
        yaxis=dict(title="% Stocks Above MA", range=[0, 100], gridcolor="#f1f5f9", ticksuffix="%"),
...
def render_hl_timeseries_chart(hl_df: pd.DataFrame, window_label: str = "52W", is_pct: bool = True) -> None:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            line=dict(color="#e11d48", width=1.8),
            fill="tozeroy",
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#ffffff",
        font=dict(family="Plus Jakarta Sans, sans-serif", size=11, color="#334155"),
        title=dict(text=f"<b>Daily New {window_label} Highs & Lows</b>", font=dict(size=14, color="#0f172a")),
        yaxis=dict(title="% of Universe" if is_pct else "Stock Count", gridcolor="#f1f5f9", ticksuffix=y_suf, range=[-y_max, y_max]),
        xaxis=dict(gridcolor="#f1f5f9"),
        legend=dict(
            orientation="h",
            yanchor="bottom",
...
def render_backtest_equity_chart(equity_curve: pd.Series, benchmark: pd.Series) -> None:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#ffffff",
        font=dict(family="Plus Jakarta Sans, sans-serif", size=11, color="#334155"),
        title=dict(text="<b>Cumulative Return: Strategy (Net) vs Benchmark</b>", font=dict(size=14, color="#0f172a")),
        yaxis=dict(title="Return %", gridcolor="#f1f5f9", ticksuffix="%"),
        xaxis=dict(gridcolor="#f1f5f9"),
        legend=dict(
            orientation="h",
            yanchor="bottom",
...
def render_multi_strategy_growth_chart(strat_curves: Dict[str, pd.Series]) -> None:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#ffffff",
        font=dict(family="Plus Jakarta Sans, sans-serif", size=11, color="#334155"),
        title=dict(text="<b>6-Month Cumulative Growth: Strategy Engines vs Benchmark</b>", font=dict(size=14, color="#0f172a")),
        yaxis=dict(title="Cumulative Return %", gridcolor="#f1f5f9", ticksuffix="%"),
        xaxis=dict(gridcolor="#f1f5f9"),
        legend=dict(
            orientation="h",
            yanchor="bottom",
