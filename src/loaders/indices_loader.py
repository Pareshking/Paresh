...
def _fetch_indices_impl(selected_indices: list[str] | None = None) -> pd.DataFrame:
    ...
    if not meta.get("last_synced") and not full.empty:
        _save_sync_metadata({
            ...
            "indices": {k: {"count": len(full), "status": "Loaded"} for k in INDICES_URLS},
        })
...
def fetch_indices_data(selected_indices: list[str] | None = None) -> pd.DataFrame:
