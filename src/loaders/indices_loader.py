...
from src.core.config import DATA_DIR, HTTP_HEADERS, INDICES_LOCAL, INDICES_URLS, REPO_DATA_DIR, SHORT_FORMS
                ...
                if resp.status_code == 200 and len(resp.content) > 200:
                    ...
                    if sym_col and len(df) > 0:
                        ...
                        if local_path:
                            ...
                            logger.info(f"[SYNC] Synced {idx_name} ({len(df)} constituents) -> {local_path}")
                            results["downloaded"][idx_name] = {
                                "count": len(df),
                                "status": "Updated",
                                "time": datetime.now().isoformat(),
                            }
                            total_unique_stocks.update(df[sym_col].astype(str).str.strip().str.upper())
                            downloaded_ok = True
                            break
            except Exception as e:
...
def _fetch_indices_impl(selected_indices: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    for idx_name, url in INDICES_URLS.items():
        ...
        for attempt in range(3):
            try:
                ...
                if resp.status_code == 200 and len(resp.content) > 200:
                    ...
                    break
                logger.warning(f"{idx_name}: HTTP {resp.status_code} (attempt {attempt+1}/3)")
            except requests.RequestException as e:
                logger.warning(f"{idx_name}: request error (attempt {attempt+1}/3): {e}")
            if attempt < 2:
        ...
        if csv_text is None:
            logger.error(f"{idx_name}: failed after 3 attempts + no local fallback — skipping")
        ...
        try:
            ...
            sym_col = next((c for c in df.columns if c.lower() in ("symbol", "ticker")), None)
            ind_col = next(
                (c for c in df.columns if c.lower() in ("industry", "sector", "macro economic sector")),
                None,
            )
            if not (sym_col and ind_col):
                logger.warning(f"{idx_name}: columns not recognized: {list(df.columns[:5])}")
                continue
            ...
            for _, row in df.iterrows():
                ...
                if symbol.startswith("DUMMY") or len(symbol) < 2 or symbol == "NAN":
                    ...
                    continue
                comp_name = str(row[name_col]).strip() if name_col and pd.notna(row[name_col]) else symbol
    ...
    if nse_failed:
        logger.warning(f"NSE blocked {len(nse_failed)} indices: {', '.join(nse_failed)}")
    ...
    for sym, info in stock_map.items():
        tags = sorted(SHORT_FORMS.get(i, i) for i in info["indices"] if SHORT_FORMS.get(i, i))
        rows.append({
            "Symbol": sym,
    ...
    logger.info(f"Loaded total universe: {len(full)} unique stocks from {len(INDICES_URLS)} indices")
    ...
    if not meta.get("last_synced") and not full.empty:
        _save_sync_metadata({
            ...
            "indices": {k: {"count": len(full), "status": "Loaded"} for k in INDICES_URLS.keys()},
        })
    ...
    keep = full["Symbol"].map(
        lambda s: bool(stock_map.get(s, {"indices": set()})["indices"] & set(selected_indices))
    )
    filtered = full[keep].reset_index(drop=True)
    logger.info(f"Selected indices filter: {len(filtered)} stocks matching {selected_indices}")
    return filtered
...
def fetch_indices_data(selected_indices: Optional[List[str]] = None) -> pd.DataFrame:
