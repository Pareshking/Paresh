...
def _fetch_single_day(target_date: datetime) -> tuple[datetime, pd.DataFrame | None]:
...
def fetch_delivery_data(
    ...
    if not all_data:
        # Fallback: use existing cached data if available, even if stale
        ...
        logger.error("No delivery data fetched and no cache available.")
        return pd.DataFrame()
