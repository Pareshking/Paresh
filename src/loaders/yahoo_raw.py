"""Raw Yahoo OHLCV acquisition and read-time project adjustment.

This module is deliberately separate from the canonical adjusted-price loader.
It captures the vendor's unadjusted OHLCV bytes and only applies the project's
corporate-action layer when a caller explicitly asks for an adjusted frame.

No ranking, benchmark, universe, or System-1 logic lives here.
"""
...
def read_raw_ohlcv(path: str | Path) -> pd.DataFrame:
    """Read a previously captured raw OHLCV parquet snapshot."""
    return _normalise_batch(pd.read_parquet(path))


def adjusted_from_raw(
    raw: pd.DataFrame, symbols: Sequence[str] | None = None,
):
    """Return the V1-style price frames after in-memory project adjustment."""
    ...
    return (
        adjusted["adj_close"], adjusted["close"], adjusted["high"],
        adjusted["low"], volume, applied,
    )
