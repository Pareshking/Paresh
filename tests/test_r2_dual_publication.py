...
def test_r2_live_archive_audit_workflow_is_read_only_and_targets_screener():
    ...
    run = str(next(s for s in steps if s.get("name") == "Audit latest Screener R2 publication")["run"])
    assert "scripts/r2_audit.py --dataset prices/screener" in run
    env = next(s for s in steps if s.get("name") == "Audit latest Screener R2 publication")["env"]
    assert "R2_SECRET_ACCESS_KEY" in env
