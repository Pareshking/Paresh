"""Executable Stage-4B Batch-1 acceptance test for DIVISLAB."""
...
from agent.divislab_research_packet import CUTOFF, divislab_packet, divislab_plan
...
def _snapshot():
    ...
    return QuantSnapshot(
        ...
        rows=({"Symbol": "DIVISLAB", "Rank": 1, "Score": 0.0},),
    )


def test_divislab_packet_is_executable():
    packet = divislab_packet()
    dossier = execute_research(
        _snapshot(),
        ResearchCandidate("DIVISLAB", 1, 0.0, {"Symbol": "DIVISLAB", "Rank": 1, "Score": 0.0}),
        packet,
    )
...
def test_divislab_packet_uses_every_evidence_record():
    packet = divislab_packet()
