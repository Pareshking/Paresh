...
def future_return(prices,end,months):
    step={1:21,3:63,6:126}[months]
    if end+step>=len(prices): return pd.Series(np.nan,index=prices.columns)
    return prices.iloc[end+step]/prices.iloc[end]-1


def month_start_indices(prices):
    """First available trading day of each calendar month."""
    grouped=prices.groupby(prices.index.to_period('M'))
    return [x.index[0] for _,x in grouped]


def main():
    OUT.mkdir(parents=True,exist_ok=True); prices=download(universe())

    # Exactly the latest six completed/current calendar months, with one
    # snapshot at the first available trading day of each month.
    months=sorted(prices.index.to_period('M').unique())[-6:]
    starts=[]
    for m in months:
        x=prices.loc[prices.index.to_period('M')==m]
        if len(x): starts.append(x.index[0])
    ends=[prices.index.get_loc(d) for d in starts]
    # A 12M formation window is still required by the tested signals, but
    # forward performance is deliberately limited to 1M/3M/6M only.
    ends=[e for e in ends if e>=260]
    ...
    for model,g in d.groupby('model'):
        ...
        rows.append(o)
    s=pd.DataFrame(rows).sort_values('rank_ic_6m',ascending=False)
    s.to_csv(OUT/'v1_hypothesis_summary.csv',index=False); d.to_csv(OUT/'v1_hypothesis_detail.csv',index=False); print(s.to_string(index=False))
