...
| Stock-specific forward-fill | Could manufacture zero returns and smooth signals | 🟢 Closed, one bounded exception | Security-specific missing observations remain missing in quantitative calculations. Exception, owner decision 2B (2026-09-25): on the NEWEST session only, up to 5 symbols that printed within the previous 5 sessions are ranked on that last print, and flagged ⏸. It is all or nothing and never applies to history (`pipeline.carried_symbols`) |
...
| Portfolio ERC naming | Lower bound means constrained ERC | 🟢 Moot | ERC (and its Ledoit-Wolf covariance) was removed from the portfolio engine: nothing in the app could reach it (`src/engine/portfolio.py` docstring), and scipy went with it |
