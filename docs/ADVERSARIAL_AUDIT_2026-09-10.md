...
| **F12** | **Medium** | `tests/test_card_symbol_links_to_the_stock_page` patched `st.html` while the renderer moved to `st.markdown` in **16206a4 (2026-09-04)**. It captured nothing and asserted against `""`. `v1-full-validation.yml` runs pytest on every push to main, so **main had been failing its own validation gate for six days.** | Reproduced: patching `st.markdown` instead, the `href` is present and the feature works. The test, not the product, was broken. | Both sinks patched, so moving the sink again cannot silently blind it. |
...
- Full suite: **666 passed, 0 failed** (from 651 passed, 1 failed).
