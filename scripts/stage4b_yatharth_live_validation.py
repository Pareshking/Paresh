"""Live hospital-operator-archetype acceptance execution for the adaptive Stage-4B engine."""
...
from agent.yatharth_research_packet import CUTOFF, yatharth_packet

snapshot = load_quant_snapshot(expected_as_of=CUTOFF)
candidate = next((c for c in top_candidates(snapshot, limit=25) if c.symbol == "YATHARTH"), None)
if candidate is None:
    raise RuntimeError("YATHARTH is not present in the canonical Top-25 snapshot")

dossier = execute_research(snapshot, candidate, yatharth_packet())
judge_dossier(dossier)

out = Path("artifacts/stage4b_yatharth_dossier.md")
out.parent.mkdir(parents=True, exist_ok=True)
lines = [
    "# Stage-4B Adaptive Hospital-Archetype Dossier — YATHARTH",
    "",
...
lines.append("STAGE4B_YATHARTH_EXECUTION=PASS")
out.write_text("\n".join(lines), encoding="utf-8")

print(f"STAGE4B_YATHARTH_AS_OF={snapshot.as_of.isoformat()}")
print(f"STAGE4B_YATHARTH_RANK={candidate.rank}")
print(f"STAGE4B_YATHARTH_SCORE={candidate.score:.6f}")
print(f"STAGE4B_YATHARTH_EVIDENCE={dossier.audit.evidence_count}")
print(f"STAGE4B_YATHARTH_PRIMARY={dossier.audit.primary_evidence_count}")
print(f"STAGE4B_YATHARTH_DOMAINS={dossier.audit.domain_count}")
print(f"STAGE4B_YATHARTH_CAUSAL={dossier.audit.causal_finding_count}")
print(f"STAGE4B_YATHARTH_CONTRADICTIONS={dossier.audit.contradiction_count}")
print(f"STAGE4B_YATHARTH_EVIDENCE_WINDOW_GAP_DAYS={dossier.evidence_window_gap_days}")
print(
    "STAGE4B_YATHARTH_EVIDENCE_AGE_BUCKETS="
...
print(
    "STAGE4B_YATHARTH_ABSENCE_BASED_FINDINGS="
    f"{dossier.audit.causal_findings_absence_based}/"
    f"{dossier.audit.contradictions_absence_based}"
)
print(f"STAGE4B_YATHARTH_NUMERIC_DISAGREEMENTS={len(dossier.audit.numeric_disagreements)}")
for item in dossier.audit.numeric_disagreements:
    print(f"STAGE4B_YATHARTH_NUMERIC_DISAGREEMENT_DETAIL={item}")
print(f"STAGE4B_YATHARTH_STALE_EVIDENCE={len(dossier.audit.stale_evidence)}")
for item in dossier.audit.stale_evidence:
    print(f"STAGE4B_YATHARTH_STALE_EVIDENCE_DETAIL={item}")
print(
    "STAGE4B_YATHARTH_STALE_ONLY_FINDINGS="
    f"{dossier.audit.causal_findings_stale_only}/"
    f"{dossier.audit.contradictions_stale_only}"
)
print(f"STAGE4B_YATHARTH_SINGLE_SOURCED_DOMAINS={len(dossier.audit.domains_single_sourced)}")
for item in dossier.audit.domains_single_sourced:
    print(f"STAGE4B_YATHARTH_SINGLE_SOURCED_DOMAIN_DETAIL={item}")
print(
    "STAGE4B_YATHARTH_SINGLE_SOURCED_FINDINGS="
    f"{dossier.audit.causal_findings_single_sourced}/"
    f"{dossier.audit.contradictions_single_sourced}"
)
print("STAGE4B_YATHARTH_EXECUTION=PASS")
