"""Live second-archetype acceptance execution for the adaptive Stage-4B engine."""
...
from agent.anandrathi_research_packet import CUTOFF, anandrathi_packet

snapshot = load_quant_snapshot(expected_as_of=CUTOFF)
candidate = next((c for c in top_candidates(snapshot, limit=25) if c.symbol == "ANANDRATHI"), None)
if candidate is None:
    raise RuntimeError("ANANDRATHI is not present in the canonical Top-25 snapshot")

dossier = execute_research(snapshot, candidate, anandrathi_packet())
judge_dossier(dossier)

out = Path("artifacts/stage4b_anandrathi_dossier.md")
out.parent.mkdir(parents=True, exist_ok=True)
lines = [
    "# Stage-4B Adaptive Second-Archetype Dossier — ANANDRATHI",
    "",
...
lines.append("STAGE4B_ANANDRATHI_EXECUTION=PASS")
out.write_text("\n".join(lines), encoding="utf-8")

print(f"STAGE4B_ANANDRATHI_AS_OF={snapshot.as_of.isoformat()}")
print(f"STAGE4B_ANANDRATHI_RANK={candidate.rank}")
print(f"STAGE4B_ANANDRATHI_SCORE={candidate.score:.6f}")
print(f"STAGE4B_ANANDRATHI_EVIDENCE={dossier.audit.evidence_count}")
print(f"STAGE4B_ANANDRATHI_PRIMARY={dossier.audit.primary_evidence_count}")
print(f"STAGE4B_ANANDRATHI_DOMAINS={dossier.audit.domain_count}")
...
print(f"STAGE4B_ANANDRATHI_STALE_EVIDENCE={len(dossier.audit.stale_evidence)}")
for item in dossier.audit.stale_evidence:
    print(f"STAGE4B_ANANDRATHI_STALE_EVIDENCE_DETAIL={item}")
print(
    "STAGE4B_ANANDRATHI_STALE_ONLY_FINDINGS="
    f"{dossier.audit.causal_findings_stale_only}/"
    f"{dossier.audit.contradictions_stale_only}"
)
print(f"STAGE4B_ANANDRATHI_SINGLE_SOURCED_DOMAINS={len(dossier.audit.domains_single_sourced)}")
for item in dossier.audit.domains_single_sourced:
    print(f"STAGE4B_ANANDRATHI_SINGLE_SOURCED_DOMAIN_DETAIL={item}")
print(
    "STAGE4B_ANANDRATHI_SINGLE_SOURCED_FINDINGS="
