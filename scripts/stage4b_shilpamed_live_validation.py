"""Live canonical-snapshot execution for the SHILPAMED Stage-4B packet."""
...
from agent.shilpamed_research_packet import CUTOFF, shilpamed_packet

snapshot = load_quant_snapshot(expected_as_of=CUTOFF)
candidate = next((c for c in top_candidates(snapshot, limit=25) if c.symbol == "SHILPAMED"), None)
if candidate is None:
    raise RuntimeError("SHILPAMED is not present in the canonical Top-25 snapshot")

dossier = execute_research(snapshot, candidate, shilpamed_packet())
judge_dossier(dossier)

out = Path("artifacts/stage4b_shilpamed_dossier.md")
out.parent.mkdir(parents=True, exist_ok=True)
lines = [
    "# Stage-4B Batch-1 Dossier — SHILPAMED",
    "",
...
lines.extend(["", "STAGE4B_SHILPAMED_EXECUTION=PASS"])
out.write_text("\n".join(lines), encoding="utf-8")

print(f"STAGE4B_SHILPAMED_AS_OF={snapshot.as_of.isoformat()}")
print(f"STAGE4B_SHILPAMED_RANK={candidate.rank}")
print(f"STAGE4B_SHILPAMED_SCORE={candidate.score:.6f}")
print(f"STAGE4B_SHILPAMED_EVIDENCE={dossier.audit.evidence_count}")
print(f"STAGE4B_SHILPAMED_PRIMARY={dossier.audit.primary_evidence_count}")
print(f"STAGE4B_SHILPAMED_DOMAINS={dossier.audit.domain_count}")
print(f"STAGE4B_SHILPAMED_CAUSAL={dossier.audit.causal_finding_count}")
print(f"STAGE4B_SHILPAMED_CONTRADICTIONS={dossier.audit.contradiction_count}")
print(f"STAGE4B_SHILPAMED_COUNTER_EVIDENCE={dossier.audit.counter_evidence_count}")
print(f"STAGE4B_SHILPAMED_EXECUTION=PASS")
