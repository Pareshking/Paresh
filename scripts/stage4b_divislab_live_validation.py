"""Live canonical-snapshot execution for the DIVISLAB Stage-4B packet."""
...
from agent.divislab_research_packet import CUTOFF, divislab_packet

snapshot = load_quant_snapshot(expected_as_of=CUTOFF)
candidate = next((c for c in top_candidates(snapshot, limit=25) if c.symbol == "DIVISLAB"), None)
if candidate is None:
    raise RuntimeError("DIVISLAB is not present in the canonical Top-25 snapshot")

dossier = execute_research(snapshot, candidate, divislab_packet())
judge_dossier(dossier)

out = Path("artifacts/stage4b_divislab_dossier.md")
out.parent.mkdir(parents=True, exist_ok=True)
lines = [
    "# Stage-4B Batch-1 Dossier — DIVISLAB",
    "",
...
lines.extend(["", "STAGE4B_DIVISLAB_EXECUTION=PASS"])
out.write_text("\n".join(lines), encoding="utf-8")

print(f"STAGE4B_DIVISLAB_AS_OF={snapshot.as_of.isoformat()}")
print(f"STAGE4B_DIVISLAB_RANK={candidate.rank}")
print(f"STAGE4B_DIVISLAB_SCORE={candidate.score:.6f}")
print(f"STAGE4B_DIVISLAB_EVIDENCE={dossier.audit.evidence_count}")
print(f"STAGE4B_DIVISLAB_PRIMARY={dossier.audit.primary_evidence_count}")
print(f"STAGE4B_DIVISLAB_DOMAINS={dossier.audit.domain_count}")
print(f"STAGE4B_DIVISLAB_CAUSAL={dossier.audit.causal_finding_count}")
print(f"STAGE4B_DIVISLAB_CONTRADICTIONS={dossier.audit.contradiction_count}")
print(f"STAGE4B_DIVISLAB_COUNTER_EVIDENCE={dossier.audit.counter_evidence_count}")
print(f"STAGE4B_DIVISLAB_EXECUTION=PASS")
